...
//        start= new StartNifty(nifty,assetManager, guiNode, guiFont, settings.getWidth(), settings.getHeight());
        start= new StartNifty(nifty);
...
//        cam3Persona.setDefaultDistance(20);
    ...
    public void onAction(String name, boolean isPressed, float tpf) {
        if (name.equals("Izquierda")) {
            ...
            Audio.playPropulsor(assetManager).playInstance();
...
//        Audio.playChimenea(assetManager);
    ...
    public void collision(PhysicsCollisionEvent event) {
        ...
        if (event.getNodeA().getName().equals("Bala de captura")) {
            ...
            Audio.playExplosion(assetManager).playInstance();
    ...
    public void simpleUpdate(float tpf) {
        ...
        if (girarDer) {
            cam3Persona.setDefaultHorizontalRotation(cam3Persona.getHorizontalRotation() + (float) Math.toRadians(2));
            fisicaPersonaje.setViewDirection(camDir);
        }
        if (girarIzq) {
            cam3Persona.setDefaultHorizontalRotation(cam3Persona.getHorizontalRotation() + (float) Math.toRadians(-2));
        ...
        if (girarArriba) {
            cam3Persona.setDefaultVerticalRotation(cam3Persona.getVerticalRotation() + (float) Math.toRadians(2));
        }
        if (girarAbajo) {
            cam3Persona.setDefaultVerticalRotation(cam3Persona.getVerticalRotation() + (float) Math.toRadians(-2));
        }
